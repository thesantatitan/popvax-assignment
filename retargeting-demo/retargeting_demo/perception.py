...
from rtmlib import Custom, Wholebody3d, draw_skeleton
...
from roi_tracker import PersistentRoiPoseTracker  # noqa: E402
from tensorrt_backend import TensorRTWholebody3d  # noqa: E402
...
def _make_tracker() -> tuple[PersistentRoiPoseTracker, str, str]:
    ...
    if backend == "tensorrt" and device != "cuda":
        backend = "onnxruntime"
        print("TensorRT requires CUDA; falling back to ONNX Runtime", flush=True)
    ...
    with target_log.open("a", encoding="utf-8", buffering=1) as output:
        while not stop_event.is_set():
            ...
            try:
                ...
                keypoints_3d, scores_raw, simcc_raw, keypoints_2d_raw = result
